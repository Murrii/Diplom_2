...
class TestRegister:
    ...
    def test_create_user_with_valid_unic_data_status_code_200(self, generate_register_data):
        new_user = RegisterMethods(generate_register_data)
    ...
    def test_create_not_unic_user_status_code_403(self, user):
        # пытаемся создать пользователя с теми же данными еще раз
        status_code, json = user.register_new_user()
    ...
    def test_create_user_without_one_required_field_status_code_403(self, not_valid_payload, generate_register_data):
        new_user = RegisterMethods(generate_register_data)
